...
public class CardServer extends TcpBridge implements TcpBridgeListener
{
  ...
  public ArrayList<CardTable> tableList = new ArrayList<CardTable>();
  ...
  private Hand talon = CardTable.deck.dealingOut(0, 0)[0]; // All cards, shuffled
  ...
  public void pipeRequest(String source, String destination, int[] indata)
  {
    switch (indata[0])
    {
      ...
      case CardPlayer.Command.TRUMP_SET:
    	  System.out.println("Got TRUMP_SET");
    	  System.out.println("Trump index: " + indata[1]);
    	  sendCommandToGroup(serverName, playerList, 100,
    		      CardPlayer.Command.SHOW_TRUMP, indata[1]);
    	  sendLastTalon();
      break;
    }
  }
  ...
  {
    int[] cardNumbers = new int[talon.getNumberOfCards()/2];
    for (int i = 0; i < talon.getNumberOfCards()/2;  i++)
      cardNumbers[i] = talon.get(i).getCardNumber();
    sendCommandToGroup(serverName, playerList, 100,
      CardPlayer.Command.TALON_DATA, cardNumbers);
    tableList.get(turnPlayerIndex).initToolBar();
  }
  
  private void sendLastTalon()
  {
    int[] cardNumbers = new int[talon.getNumberOfCards()/2];
    for (int i = 0; i < talon.getNumberOfCards()/2; i++)
      cardNumbers[i] = talon.get(i+12).getCardNumber();
    sendCommandToGroup(serverName, playerList, 100,
      CardPlayer.Command.TRUMP_SET, cardNumbers);
  }
}